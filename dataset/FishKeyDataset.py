...
class FishKeyPoseDataset(JointsDataset):
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        self.num_joints = 11
        self.flip_pairs = [[0, 1], [1, 2], [0, 3], [3, 4], [3, 5],
                           [3, 6], [3, 9], [7, 9], [8, 9], [9, 10]]
    ...
    def _get_ann_file_keypoint(self):
        """ self.root / annotations / model_pose_train.json """
        prefix = 'fish_keypoints'
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        for obj in objs:
            ...
            joints_3d = np.zeros((self.num_joints, 3), dtype=np.float32)        # 由于np.float的弃用我改成了np.float32
            joints_3d_vis = np.zeros((self.num_joints, 3), dtype=np.float32)    # 由于np.float的弃用我改成了np.float32
    ...
    def _load_coco_person_detection_results(self):
        ...
        for n_img in range(0, len(all_boxes)):
            ...
            joints_3d_vis = np.ones(
                (self.num_joints, 3), dtype=np.float32)     # 由于np.float的弃用我改成了np.float32
            kpt_db.append({
                'image': img_name,
                'center': center,
                'scale': scale,
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path,
        ...
        for idx, kpt in enumerate(preds):
            ...
            # file_text = os.path.splitext(file_name)[0]
            _kpts.append({
                'keypoints': kpt[:, :3],
        ...
        for img in kpts.keys():
            ...
            oks_nmsed_kpts.append([img_kpts[0]])
    ...
    def _coco_keypoint_results_one_category_kernel(self, data_pack):
        ...
        for img_kpts in keypoints:
            ...
            key_points = np.zeros(
                (_key_points.shape[0], self.num_joints * 3), dtype=np.float32)  # 由于np.float的弃用我改成了np.float32
...
#
# annFile = r'../data/annotations/fish_keypoints_valid.json'  # 转换之后的json文件路径
# img_prefix = '../data/images'  # 图片文件夹路径
#
